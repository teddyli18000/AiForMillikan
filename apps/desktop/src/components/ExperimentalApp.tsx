...
export function ExperimentalApp() {
  ...
  const [message, setMessage] = useState("Experimental 准备就绪");
  ...
  const openRunFolder = async () => {
    if (artifacts?.run_dir) await desktopApi.openPath(artifacts.run_dir);
  };
  ...
  return (
    <div className="desktop-frame">
      ...
      <div className="status-toast" role="status">{message}</div>
    </div>
  );
}
