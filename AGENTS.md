...
- After a user accepts a Normal q record, the UI must offer a clear "next droplet" action. Choosing the same video returns to target selection with the confirmed boundary and video context preserved but clears the in-progress target/tracking/review/q draft. Choosing a different video returns to the empty import state while preserving accepted records in the current transient session for cross-video inversion.
