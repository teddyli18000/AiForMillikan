...
本系统面向密立根油滴实验的视频数据。Normal 主线中，用户上传或拖入包含仪器显示屏的实验视频后，系统自动检查视频信息、建议 `0 V` 下落时间窗口、识别网格线、追踪用户框选的一颗油滴，并在物理约束下计算该油滴的电荷量。用户保留或重测该记录，在本次应用启动内跨一个或多个视频积累至少三颗有效油滴后，再进入基本电荷盲反演。应用重启后 Normal 从全新测量 session 开始，不自动加载旧记录；长期保留依赖用户显式导出。
...
| 仪器参数 | 极板间距 $d$、测量距离 $l$、油滴密度 $\rho$、空气粘滞系数 $\eta$、气压 $p$、Cunningham 修正常数等 | 默认来自后端配置；Normal UI 放在折叠高级区，覆盖只影响本次测量；气压单位为 `kPa` |
...
- 默认重力加速度：$g=9.79\,\mathrm{m/s^2}$；
- 默认空气黏滞系数：$\eta=1.83\times10^{-5}\,\mathrm{Pa\,s}$；
- 默认气压：$p=101.325\,\mathrm{kPa}$；
...
+ 一个可信平衡电压 U_bal
+ 一个用户确认的 0V 下落时间窗口
+ 用户确认 0V_start_s 前后 0.5 s 内的实际框选帧和矩形框
+ 从实际框选帧开始的可拟合单滴轨迹
+ 网格尺度有效，或用户修正后有效
```
...
flowchart TD
    ...
    K --> L[用户选择保留或返回调整]
    L --> M[本次启动的 Normal session 累积跨视频 q 记录]
```
...
Normal v1 固定采用这个 special case。程序要求用户明确确认目标油滴在
输入的平衡电压下处于平衡或近似平衡状态；程序不声称自动验证了平衡。
`0 V` 下落速度来自用户确认的 `0V_start_s` 到 `0V_end_s` 内、从实际
框选帧开始追踪得到的轨迹。目标选择帧只能在用户确认的
`0V_start_s ± 0.5 s` 内。
...
对 Normal v1 的 `0 V` 下落段，后端必须从线性回归直接计算斜率标准误差，
不能用 RMSE、位移和 $R^2$ 拼接经验公式。设 $s$ 为像素斜率
（$\mathrm{px/s}$），$N$ 为参与拟合的有效点数，$SSR$ 为残差平方和：

$$
\sigma_s^2 =
\frac{SSR}{N-2}
\frac{1}{\sum_i(t_i-\bar t)^2}
$$
...
其中 $s_y$ 是纵向像素到物理距离的比例。若 $N\le2$、时间跨度为零、
或 $\sigma_v$ 非有限/非正，则该 record 只能作为 diagnostic，不能进入
盲反演。
...
Normal v1 当前只把上述速度拟合随机不确定度传播到
`sigma_q_C`。电压、标尺、极板间距、黏度、气压、密度和 Cunningham 参数
的不确定度，只有在配置或文档明确给出来源后才能纳入；未纳入时必须在
结果中透明标记。

由于 Cunningham 修正下 $q$ 是 $v$ 的非线性函数，不能简单令
$\sigma_q/q=\sigma_v/v$。令：

$$
B=\frac{b}{p}
$$

则 Normal v1 使用的局部对数灵敏度为：
...
旧版本的 `max(5%, empirical_RMSE_R2_term)` 不能再作为正式
