...
def calibrate_grid(video_path: str | Path, cfg: dict[str, Any], start_frame: int = 0, end_frame: int | None = None, progress_callback: ProgressCallback | None = None) -> dict[str, Any]:
    ...
    scale = float(cfg["grid"].get("measurement_distance_m", 0.0015)) / float(span)
    mask = build_grid_mask(gray, lines_y, int(cfg["grid"].get("mask_dilate_px", 5)))
    return {
        "valid": True,
        "grid_lines_y": lines_y,
        "second_line_y": second,
        "penultimate_line_y": penultimate,
        "measurement_distance_m": float(cfg["grid"].get("measurement_distance_m", 0.0015)),
...
def build_grid_mask(gray: np.ndarray, lines_y: list[int], dilate_px: int = 5) -> np.ndarray:
